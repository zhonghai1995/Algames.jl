...
#Example from TrajectoryOptimization package
#```julia
...
function add_goal_constraint!(game_con::GameConstraintValues, xg::AbstractVector{T})  where {T}
	probsize = game_con.probsize
	N = probsize.N
	n = probsize.n
	m = probsize.m
	...
	add_constraint!(
		game_con.goal_conlist,
		TrajectoryOptimization.GoalConstraint(n,xg,SVector{n,Int}(1:length(xg))),
		N)
	con  = game_con.goal_conlist.constraints[end]
	inds = game_con.goal_conlist.inds[end]
	print(inds)
	conval = Altro.ALConVal(n,m,con,inds)
	push!(game_con.goal_conval, conval)
	return nothing
end
